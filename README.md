...
>> 각 가게주를 구별하기위해 회원가입할 때 사용자마다 부여되었던 uid를 key값으로 설정했습니다.
>> uid로 가게주를 구별하며 안에는 가게 이름, 주소, 영업시간에 대한 정보가 저장됩니다.
...
>> 저희가 아직 가게 정보를 입력 받을 시점을 정하지 않았습니다. 그래서 일단 가게 데이터가 비어있을 시에 정보 수정을 사용하면 데이터가 저장되게 하였습니다.
>> 이는 다음주 중에 팀원들과 입력 받을 시점을 정한 후에 수정할 계획입니다. 
>> 다음은 그 실행영상입니다.
